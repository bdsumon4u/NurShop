...
class ProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $product->related()->attach($request->relatedProducts);
    ...
    {
        $product = Product::with('media','categories','related')->where('products.id','=',$id)->first();
        $images = DB::table('media')
            ->join('media_product', 'media.id', '=', 'media_product.media_id')
            ->where('media_product.product_id','=',$id)
    ...
    public function update(Request $request, $id)
    {
        ...
        $product->related()->sync($request->relatedProducts);
...
//            if(empty($checkProduct)) {
                ...
                $newProduct->ProductName = str_replace('&amp;','&',htmlspecialchars($syncProduct->get_name));
                $newProduct->productSlug = $this->slug(str_replace('&amp;','&',htmlspecialchars($syncProduct->get_name)));
                ...
                foreach ($syncProduct->get_categories as $categories) {
                    ...
                    if (!empty($category)) {
                        ...
                        $category->categoryName = str_replace('&amp;','&',$categories->name);
                        $category->categorySlug = $this->slug(str_replace('&amp;','&',$categories->name));
...
//            }
    ...
    public function oldProductSync(Request $request)
    {
        ...
        $curl = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_URL => "https://dorkarishop.com/wp-json/inventory/v1/products/",
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
     ...
     public function api(){
        ...
        return json_encode(Product::with('media','categories')->get());
        
    }
        public function relatedProducts()
    {
        if(isset($request['q'])){
            $products = Product::query()->where('productName','like','%'.$request['q'].'%')->select('id','productName as text')->get();
        }else{
            $products = Product::select('id','productName as text')->get();
        }
