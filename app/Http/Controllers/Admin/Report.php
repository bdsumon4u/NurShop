...
class Report extends Controller
{
    ...
    public function getProduct(Request $request){
        ...
        $orders = DB::table('orders')
            ...
            if($status != 'All' && $status != 'Pending Invoiced' && $status != 'Delivered' && $status != 'Payment Pending' && $status != 'On Hold' && $status != 'Request to Return'){
            ...
            if($status == 'On Hold'){
                $orders  = $orders->where('orders.status', 'like', 'On Hold');
            }
            
            if($status == 'Payment Pending'){
                $orders  = $orders->where('orders.status', 'like', 'Payment Pending');
            }
            
            if ($status == 'Delivered') {
                $orders = $orders->whereIn('orders.status', ['Delivered', 'Customer Confirm','Customer On Hold','Request to Return']);
            }
            if ($status == 'Request to Return') {
                $orders  = $orders->where('orders.status', 'like', 'Request to Return');
            }
            if($request['courierID'] != ''){
                $orders = $orders->where('orders.courier_id','=',$request['courierID']);
            }
